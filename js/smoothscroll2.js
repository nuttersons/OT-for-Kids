...
var Document = new Native({
	...
	initialize: function(doc){
		...
		doc.php = doc.getElementsByTagName('html')[0];
...
var Event = new Native({
	...
	initialize: function(event, win){
		...
		if (type.test(/key/)){
			...
			doc = (!doc.compatMode || doc.compatMode == 'CSS1Compat') ? doc.php : doc.body;
			var page = {
...
Element.Properties.php = {set: function(){
...
Credits:
	- Element positioning based on the [qooxdoo](http://qooxdoo.org/) code and smart browser fixes, [LGPL License](http://www.gnu.org/licenses/lgpl.php).
	- Viewport dimensions based on [YUI](http://developer.yahoo.com/yui/) code, [BSD License](http://developer.yahoo.com/yui/license.php).
*/
...
function getCompatElement(element){
	...
	return (!doc.compatMode || doc.compatMode == 'CSS1Compat') ? doc.php : doc.body;
};
...
Request.php = new Class({
	...
	success: function(text){
		...
		response.php = text.stripScripts(function(script){
			response.javascript = script;
		});
		
		var temp = this.processHTML(response.php);
		...
		this.onSuccess(response.tree, response.elements, response.php, response.javascript);
	}
...
Element.Properties.load = {
	...
	get: function(options){
		if (options || ! this.retrieve('load')){
			...
			this.store('load', new Request.php(this.retrieve('load:options')));
		}
